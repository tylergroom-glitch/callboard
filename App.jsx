...
function RundownTab({ event, update, isAdmin, editor, showId }) {
  ...
  const thumbSize = Math.max(24, rowH - 14);
    ...
    <div className="tp-panel">
      ...
      {boxes.length ? (
        <div className="tp-plan">
          <svg ref={svgRef} viewBox={"0 0 " + svgW + " " + svgH} width="100%" style={{ maxHeight: "62vh", background: "#080b12", borderRadius: 8, touchAction: "none" }} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerLeave={endDrag}>
            {planLen > truck.len && <rect x={0} y={truck.len * scale} width={svgW} height={(planLen - truck.len) * scale} fill="rgba(248,113,113,0.06)" />}
            <rect x={1.5} y={1.5} width={truck.w * scale - 3} height={truck.len * scale - 3} fill="#0e1420" stroke="#3b82f6" strokeWidth={2.5} rx={6} />

...
function Callboard({ auth, onLogout }) {
  ...
  const calendar = (
    <ShowsCalendar
      ...
      hasSidebar={isSuperAdmin}
      onOpen={openLandingShow}
      onNew={newEvent}
      onDemo={newDemoEvent}
      onPipeline={() => goAdmin("pipeline")}
      onPeople={() => setPeopleOpen(true)}
      onCatalog={() => goAdmin("catalog")}
      onQuotes={() => goAdmin("quotes")}
      onLogout={onLogout}
    />
  );
...
function CatalogTerms() {
  ...
  const save = async () => {
    ...
    try { await saveQuoteTerms(text); setSaved(true); window.setTimeout(() => setSaved(false), 2200); }
    catch (e) { setErr((e && e.message) || "Couldn't save."); }
    setBusy(false);
  };
  ...
  return (
    <div>
      ...
      {err ? <div style={ctgErr}>{err}</div> : null}
      <textarea value={text} onChange={(e) => setText(e.target.value)} rows={22} style={{ width: "100%", fontSize: 13, lineHeight: 1.5 }} placeholder="1. Payment Terms&#10;&#10;A deposit of…" />
      <div style={{ ...ctgRow, marginTop: 12 }}>
        <button className="btn amber" onClick={save} disabled={busy}>{busy ? "Saving…" : "Save terms"}</button>
        {saved ? <span style={{ color: "var(--green)", fontSize: 13 }}>Saved</span> : null}
        <span style={{ ...ctgHint, marginLeft: "auto" }}>{text.trim() ? text.trim().split(/\s+/).length + " words" : "Empty"}</span>
      </div>
    </div>
  );
}
...
/* Two standing terms, plus building your own. Nothing is applied automatically —
   a new quote starts with no schedule at all, because plenty of them never need
   one. "Before load-in" counts back from the quote's start date and "after
   strike" counts on from its end date, so the dates follow if the show moves. */
const QT_PRESETS = [
  {
    key: "7525",
    label: "75 / 25",
    hint: "75% thirty days before, balance thirty days after",
    rows: [
      { label: "Deposit", pct: 75, trigger: "before_loadin", offset: 30 },
  ...
  },
  {
    key: "255025",
    label: "25 / 50 / 25",
    hint: "25% on signing, 50% thirty days before, balance thirty days after",
    rows: [
...
/* ---------- branded quote PDF ---------- */
/* Same machinery as the Production Brief export: build a self-contained HTML
   document, open it in a throwaway tab, let html2pdf render it, then close. */
...
function qtQuoteHtml(opts) {
  ...
  if (mode === "summary") {
    ...
    for (const g of groups) {
      const gl = lines.filter((l) => l.groupId === g.id);
      if (!gl.length) continue;
      rows += "<tr><td>" + qtEsc(g.name) + "</td><td class='n b'>" + money(gl.reduce((t, l) => t + qtLineTotal(l), 0)) + "</td></tr>";
    }
    const un = lines.filter((l) => !l.groupId || !groups.some((g) => g.id === l.groupId));
    if (un.length)
      rows += "<tr><td>" + (groups.length ? "Additional" : "Equipment &amp; Labor") + "</td><td class='n b'>" + money(un.reduce((t, l) => t + qtLineTotal(l), 0)) + "</td></tr>";
    body = "<div class='grp'><table><thead><tr><th>Description</th><th class='n'>Amount</th></tr></thead><tbody>" + rows + "</tbody></table></div>";
  }
  ...
  const termsBlock = terms.trim()
    ? "<div class='terms'><div class='sect-title'>Terms &amp; Conditions</div>" +
      terms
        .split(/\n{2,}/)
        .map((p) => "<p>" + qtEsc(p).replace(/\n/g, "<br>") + "</p>")
        .join("") +
      "</div>"
    : "";
...
function qtExportQuotePdf(opts) {
  const html = qtQuoteHtml(opts);
  ...
  if (win) {
    ...
    a.href = url;
    a.download = String(opts.quote.name || "Quotation").replace(/[^a-zA-Z0-9 _-]/g, "") + ".html";
...
/* Saved bundles of groups and the gear in them. A preset holds the recipe —
   which catalog item, how many, how many days — and never the price: rates are
   read fresh from the catalog each time one is used, so a preset cannot quietly
   quote last year's numbers. Anything typed as a blank line keeps the rate it
   was saved with, since there is no catalog row to look it up from. */
function QtPresets({ catalog, currentGroups, currentLines, busy, onApply, onClose }) {
  const [rows, setRows] = useState(null);
  const [err, setErr] = useState("");
  ...
  const load = async () => {
    try { setRows(await listQuotePresets()); setErr(""); }
    catch (e) { setErr((e && e.message) || "Couldn't load presets."); setRows([]); }
  };
  useEffect(() => { load(); }, []);

  const rateNow = (l) => {
    const hit = l.catalogId ? (catalog || []).find((c) => c.id === l.catalogId) : null;
    return hit ? hit.rate : qtNum(l.rate);
  };
  const worth = (p) => {
    const lines = (p.data && p.data.lines) || [];
    return lines.reduce((t, l) => t + Math.round(qtNum(l.qty) * qtNum(l.days) * rateNow(l) * 100) / 100, 0);
  };
  const stale = (p) => ((p.data && p.data.lines) || []).filter((l) => l.catalogId && !(catalog || []).some((c) => c.id === l.catalogId)).length;

  const saveCurrent = async () => {
    const nm = name.trim();
    if (!nm) { setErr("Give the preset a name."); return; }
    const keep = currentGroups.filter((g) => pick.has(g.id));
    if (!keep.length) { setErr("Pick at least one group to save."); return; }
    const ids = new Set(keep.map((g) => g.id));
    const lines = currentLines.filter((l) => ids.has(l.groupId));
    if (!lines.length) { setErr("Those groups have no lines in them."); return; }
    setSaving(true);
    try {
      await saveQuotePreset(nm, notes.trim(), {
        groups: keep.map((g) => ({ id: g.id, name: g.name })),
        // Deliberately no discount, vendor or sub-rental cost: those belong to
        // the job that was quoted, not to the recipe.
        lines: lines.map((l) => ({
          groupId: l.groupId, catalogId: l.catalogId || null, name: l.name,
          department: l.department || "Misc", qty: qtNum(l.qty), days: qtNum(l.days),
          rate: qtNum(l.rate), kind: l.kind || "item",
        })),
      });
      setName(""); setNotes(""); setPick(new Set()); setErr("");
      await load();
    } catch (e) { setErr((e && e.message) || "Couldn't save that preset."); }
    setSaving(false);
  };

  const remove = async (p) => {
    if (!window.confirm("Delete the preset \"" + p.name + "\"? Quotes already built from it are unaffected.")) return;
    try { await deleteQuotePreset(p.id); await load(); }
    catch (e) { setErr((e && e.message) || "Couldn't delete that preset."); }
  };

  return (
    <CtgModal title="Gear presets" onClose={onClose} wide>
      {err ? <div style={qtErr}>{err}</div> : null}

      <p style={{ ...qtHint, marginTop: 0 }}>
        A preset drops its groups and gear into this quote, on top of whatever is already here — nothing is replaced. Rates come from the
        catalog as it stands today, not from when the preset was saved.
      </p>

      {rows === null ? (
        <div style={qtHint}>Loading…</div>
      ) : !rows.length ? (
        <div style={{ ...qtHint, padding: 22, textAlign: "center" }}>No presets yet. Build a quote the way you like it, then save its groups below.</div>
      ) : (
        rows.map((p) => {
          const gs = (p.data && p.data.groups) || [];
          const ls = (p.data && p.data.lines) || [];
          const missing = stale(p);
          return (
            <div key={p.id} style={{ ...qtListRow, marginBottom: 5, alignItems: "flex-start" }}>
              <span style={{ flex: 1, minWidth: 180, display: "flex", flexDirection: "column", gap: 2 }}>
                <span style={{ fontWeight: 700, fontSize: 14 }}>{p.name}</span>
                <span style={{ fontSize: 12, color: "var(--dim)" }}>
                  {gs.length} group{gs.length === 1 ? "" : "s"} · {ls.length} line{ls.length === 1 ? "" : "s"} · {qtMoney0(worth(p))} at today's rates
                </span>
                {p.notes ? <span style={{ fontSize: 12, color: "var(--faint)" }}>{p.notes}</span> : null}
                {missing ? (
                  <span style={{ fontSize: 11, color: "#FFB020", fontWeight: 700 }}>
                    {missing} item{missing === 1 ? " is" : "s are"} no longer in the catalog — {missing === 1 ? "it keeps" : "they keep"} the saved rate
                  </span>
                ) : null}
              </span>
              <button className="btn amber" onClick={() => onApply(p)} disabled={busy} style={{ padding: "6px 12px" }}>Add to quote</button>
              <button className="btn ghost danger" onClick={() => remove(p)} disabled={busy} style={{ padding: "6px 10px" }}>Delete</button>
            </div>
          );
        })
      )}
      ...
      <div style={{ borderTop: "1px solid var(--line)", marginTop: 16, paddingTop: 14 }}>
        <div style={{ fontSize: 14, fontWeight: 800, marginBottom: 6 }}>Save from this quote</div>
        {!currentGroups.length ? (
          <p style={qtHint}>This quote has no groups yet. Presets are saved as groups, so make one first.</p>
        ) : (
          <>
            <p style={{ ...qtHint, marginTop: 0 }}>Tick the groups to keep. Discounts, vendors and sub-rental costs are left out — those belong to the job, not the recipe.</p>
            <div style={{ ...qtRow, marginBottom: 10 }}>
              {currentGroups.map((g) => {
                const n = currentLines.filter((l) => l.groupId === g.id).length;
                const on = pick.has(g.id);
                return (
                  <button
                    key={g.id}
                    onClick={() => setPick((prev) => { const nx = new Set(prev); if (nx.has(g.id)) nx.delete(g.id); else nx.add(g.id); return nx; })}
                    style={ctgChip(on, "#FFB020")}
                    title={n + " line(s)"}
                  >
                    {g.name} ({n})
                  </button>
            ...
            <div style={qtRow}>
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Preset name — e.g. General Session Audio" style={{ flex: "1 1 240px" }} />
              <input value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Note to yourself (optional)" style={{ flex: "1 1 200px" }} />
              <button className="btn" onClick={saveCurrent} disabled={saving || !pick.size}>{saving ? "Saving…" : "Save preset"}</button>
            </div>
            <p style={{ ...qtHint, marginTop: 6 }}>Saving under a name that already exists updates that preset.</p>
          </>
        )}
      </div>
    </CtgModal>
...
function QuoteEditor({ quoteId, catalog, clients, venues, onClose, onChanged, onRefs, onOpenShow, onShowCreated }) {
  ...
  /* Groups and lines come in with fresh ids so a preset can be used twice on
     ...
     taken from the catalog as it is now; a line whose catalog item has since
     been deleted falls back to what was saved with the preset. */
  const applyPreset = (p) => {
    const pg = (p.data && p.data.groups) || [];
    const pl = (p.data && p.data.lines) || [];
    if (!pl.length) { setErr("That preset has no lines in it."); return; }
    const idMap = {};
    const newGroups = pg.map((g) => {
      const nid = uid();
      idMap[g.id] = nid;
      // Two groups called the same thing on one quote reads as a mistake, so
      // a repeat is numbered rather than silently duplicated.
      const clash = groups.filter((x) => x.name === g.name || x.name.indexOf(g.name + " (") === 0).length;
      return { id: nid, name: clash ? g.name + " (" + (clash + 1) + ")" : g.name };
    });
    const newLines = pl.map((l) => {
      const hit = l.catalogId ? (catalog || []).find((c) => c.id === l.catalogId) : null;
      return {
        id: uid(),
        kind: l.kind || "item",
        groupId: idMap[l.groupId] || null,
        catalogId: l.catalogId || null,
        name: hit ? hit.name : l.name,
        department: hit ? hit.department : (l.department || "Misc"),
        qty: qtNum(l.qty) || 1,
        days: qtNum(l.days) || 1,
        rate: hit ? hit.rate : qtNum(l.rate),
        discount: 0,
        subVendor: "",
        subCost: hit && hit.subCost != null ? String(hit.subCost) : "",
      };
    });
  ...
  /* Replacing a schedule that already has money against it would restate an
     invoice that has gone out, so a paid row blocks it outright rather than
     asking — the schedule can still be edited row by row. */
  const paidRows = (data.deposits || []).filter((d) => d.paid).length;
  const applySchedule = (rows) => {
    if (paidRows) {
      setErr("This schedule has " + paidRows + " payment" + (paidRows === 1 ? "" : "s") + " already marked paid, so it can't be replaced. Edit the remaining rows instead.");
      return;
    }
    if (data.deposits.length && !window.confirm("Replace the current payment schedule?")) return;
    touch({ ...data, deposits: rows.map((d) => ({ id: uid(), label: d.label, pct: d.pct, trigger: d.trigger, offset: d.offset, dueDate: "", paid: false })) });
  };
  const startCustom = () => applySchedule([
    { label: "", pct: 50, trigger: "signature", offset: 0 },
    { label: "", pct: 50, trigger: "after_strike", offset: 30 },
  ]);
    ...
    <div className="cal-wrap">
      ...
      <div className="panel" style={{ marginTop: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, flexWrap: "wrap", gap: 8 }}>
          ...
          {!locked && (
            <div style={{ display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap" }}>
              {QT_PRESETS.map((p) => (
                <button key={p.key} className="btn ghost" onClick={() => applySchedule(p.rows)} style={{ padding: "5px 11px" }} title={p.hint} disabled={!!paidRows}>
                  {p.label}
                </button>
              ))}
              <button className="btn ghost" onClick={startCustom} style={{ padding: "5px 11px" }} title="Start from two blank rows and build your own" disabled={!!paidRows}>
                Custom
              </button>
            </div>
          )}
        ...
        {!data.deposits.length ? (
          <p style={qtHint}>
            No schedule yet. Pick <b>75 / 25</b> or <b>25 / 50 / 25</b>, or start <b>Custom</b> and build your own — every row can be
            relabelled, re-weighted and re-timed afterwards whichever you choose.
          </p>
        ) : null}
...
/* Who has full run of the platform. This is the is_tcg flag on the account —
   ...
   and P&L. Deliberately kept separate from People & Access, which is per-show. */
function TcgStaffScreen({ onClose }) {
  ...
  const load = async () => {
    setLoading(true);
    try { setPeople(await listProfiles()); setErr(""); }
    catch (e) { setErr((e && e.message) || "Couldn't load accounts."); }
    setLoading(false);
  };
  useEffect(() => { load(); }, []);

  const setFlag = async (person, isTcg) => {
    if (!isTcg && !window.confirm("Remove admin access for " + (person.email || "this person") + "?\n\nThey keep any show roles they already have.")) return;
    setBusy(person.id); setErr(""); setNotice("");
    try {
      await setTcgAdmin({ userId: person.id, isTcg });
      setNotice(isTcg ? (person.email + " is now an admin — they'll need to sign out and back in.") : ("Removed admin access for " + person.email + "."));
      await load();
    } catch (e) { setErr((e && e.message) || "Couldn't update that account."); }
    setBusy("");
  };
  ...
  const invite = async () => {
    const em = email.trim();
    if (!em) return;
    setBusy("invite"); setErr(""); setNotice("");
    try {
      const r = await setTcgAdmin({ email: em, isTcg: true });
      setNotice(r && r.invited
        ? ("Invited " + em + " — they'll get an email to set a password, then they're a full admin.")
        : (em + " is now an admin — they'll need to sign out and back in."));
      setEmail(""); await load();
    } catch (e) { setErr((e && e.message) || "Couldn't add that person."); }
    setBusy("");
  };

  const term = q.trim().toLowerCase();
  const shown = people.filter((p) => !term || (p.email || "").toLowerCase().includes(term) || (p.name || "").toLowerCase().includes(term));
  const admins = shown.filter((p) => p.is_tcg);
  const others = shown.filter((p) => !p.is_tcg);
  const inp = { flex: 1, minWidth: 180, background: "var(--panel2)", border: "1px solid var(--line)", color: "var(--ink)", borderRadius: 8, padding: "9px 11px", fontFamily: "inherit", fontSize: 14 };

  const row = (p) => (
    <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderBottom: "1px solid var(--line)" }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 14, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name || p.email || "—"}</div>
        {p.name ? <div style={{ fontSize: 12, color: "var(--dim)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.email}</div> : null}
      </div>
      {p.is_tcg ? <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--amber)" }}>Admin</span> : null}
      <button
        className={"btn " + (p.is_tcg ? "ghost" : "amber")}
        onClick={() => setFlag(p, !p.is_tcg)}
        disabled={busy === p.id}
      >
        {busy === p.id ? "\u2026" : p.is_tcg ? "Remove" : "Make admin"}
      </button>
    </div>
  );
  ...
  return (
    <div className="cal-wrap">
      <div className="cal-top">
        <h1 className="cal-h1">TCG Staff</h1>
        <div className="cal-top-actions">
          <button className="btn ghost" onClick={onClose}>Back to shows</button>
        </div>
      </div>
      ...
      <p style={{ color: "var(--dim)", fontSize: 13, marginTop: 0, maxWidth: 640 }}>
        Admins see every show plus Pipeline, Quotes, Catalog and P&amp;L. It's all or nothing — there's no way to grant Quotes without P&amp;L.
      </p>
      <p style={{ color: "var(--faint)", fontSize: 12.5, marginTop: -4, marginBottom: 16, maxWidth: 640 }}>
        Access is read when someone signs in, so a change won't show up for anyone already signed in until they sign out and back in.
      </p>

      {err ? <div style={{ background: "rgba(255,107,107,.12)", border: "1px solid var(--danger)", color: "var(--danger)", borderRadius: 8, padding: "9px 12px", fontSize: 13, marginBottom: 12 }}>{err}</div> : null}
      {notice ? <div style={{ background: "rgba(95,208,138,.12)", border: "1px solid var(--green)", color: "var(--green)", borderRadius: 8, padding: "9px 12px", fontSize: 13, marginBottom: 12 }}>{notice}</div> : null}
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginBottom: 18 }}>
        <input
          ...
          placeholder="name@touchstonecreativegroup.com"
        />
        <button className="btn amber" onClick={invite} disabled={busy === "invite" || !email.trim()}>
          {busy === "invite" ? "Adding\u2026" : "Add as admin"}
        </button>
      </div>

      {loading ? (
        <div style={{ color: "var(--dim)", padding: 30, textAlign: "center" }}>Loading accounts\u2026</div>
      ) : (
        <>
          <div className="cal-list-h">Admins ({admins.length})</div>
          <div style={{ border: "1px solid var(--line)", borderRadius: 10, overflow: "hidden", marginBottom: 22 }}>
            {admins.length ? admins.map(row) : <div style={{ padding: 16, color: "var(--dim)", fontSize: 13 }}>No admins match that search.</div>}
          </div>

          <div className="cal-list-h" style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span>Everyone else ({others.length})</span>
            <input style={{ ...inp, flex: "0 1 220px", minWidth: 140, padding: "6px 10px", fontSize: 13 }} value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search\u2026" />
          </div>
          <div style={{ border: "1px solid var(--line)", borderRadius: 10, overflow: "hidden" }}>
            {others.length ? others.map(row) : <div style={{ padding: 16, color: "var(--dim)", fontSize: 13 }}>Nobody else yet.</div>}
          </div>
        </>
      )}
    </div>
...
function ShowsCalendar({ events, isAdmin, onOpen, onNew, onDemo, onPipeline, onPeople, onCatalog, onQuotes, onLogout, hasSidebar }) {
  const now = new Date();
  ...
  return (
    <div className="cal-wrap">
      <div className="cal-top">
        ...
        <div className="cal-top-actions">
          {isAdmin && !hasSidebar && <button className="btn ghost" onClick={onPeople}>People &amp; Access</button>}
          {isAdmin && !hasSidebar && <button className="btn ghost" onClick={onPipeline}>Pipeline</button>}
          {isAdmin && !hasSidebar && <button className="btn ghost" onClick={onQuotes}>Quotes</button>}
          {isAdmin && !hasSidebar && <button className="btn ghost" onClick={onCatalog}>Catalog</button>}
...
<div class="hdr">
  <div class="hdr-logo"><img src="${TCG_LOGO}" alt="TCG"></div>
  <div class="hdr-right">
...
/* ============================================================
   ...
   Stored in its own show_costing table via /api/costing, so the figures are
   never sent to crew — only admin tokens can read or write them.
   ============================================================ */
  ...
  if (state === "error")
    return (
      <Panel title="P&L / Costing">
        <Empty>Couldn’t load costing. If this is the first time you’ve opened P&amp;L since the Supabase migration, setup-costing.sql may still need to be run.</Empty>
      </Panel>
    );
...
.cb .adm-shell{display:flex; align-items:flex-start;}
.cb .adm-side{
  position:sticky; top:0; z-index:20;
  flex:0 0 206px; width:206px; height:100vh;
